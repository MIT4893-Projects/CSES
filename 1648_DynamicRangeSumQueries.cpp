...
struct SegmentTree
{
    ...
    int Build(TreeNode *curr)
    {
        ...
        return curr->val = Build(curr->left) + Build(curr->right);
    }
    ...
    int Query(int left, int right, TreeNode *curr)
    {
        ...
        return Query(left, right, curr->left) + Query(left, right, curr->right);
    }

    int Update(int index, int val, TreeNode *curr)
    {
        int l = curr->l;
        int r = curr->r;
        ...
        return curr->val = Update(index, val, curr->left) + Update(index, val, curr->right);
    }
};
